...
class EF:
    ...
    def add(self):
      self.values[self.ptr] = self.values[self.ptr] + 1 if self.values[self.ptr] < 255 else 0

    def subtract(self):
      self.values[self.ptr] = self.values[self.ptr] - 1 if self.values[self.ptr] > 0 else 255

    def begin(self):
       if self.values[self.ptr] == 0: self.loc = self.loopLoc[self.loc]
